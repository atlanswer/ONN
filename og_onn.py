...
timestr = time.strftime(
    "%Y-%m-%d %H %M"
)  # Current time and date for the output optimization file
print(timestr)
import os
...
path = (
    path + "/save_og_onn/"
)  # Create a folder under the same directory as the optimization code where the optimization results will be saved

n_dimensions = 12  # n_dimensions of optimization benchmark function
s_init = 2 * n_dimensions  # number of initial samples (initial dataset)
num_candidates = (
    2 * n_dimensions  # number of candidate solutions (it has to be smaller than s_init)
)
num_new_candidates = 2 * n_dimensions  # new samples from the candidate solutions
...
alpha_mutation_var = 0.3  # initial mutation ratio of the n_dimensions
n_mutations = (
    int(alpha_mutation_var * n_dimensions) + 1
)  # number of dimensions to be mutated
n_opt = 1  # number of total optimization runs
...
def gen_candidates(X, Y, num_candidates):  # Step 2
    idx = np.argpartition(
        Y, num_candidates - 1
    )  # It does not sort the entire array. It only guarantees that the kth element is in sorted position and all smaller elements will be moved before it. Thus the first k elements will be the k-smallest elements.
    X_cand = []
    Y_cand = []
    ...
    low = np.amin(
        X_cand, axis=0
    )  # This is the low/high of each variable and works best for  functions with variables with different bounds (ex. antennas)
...
def mutate(candidate, low, high, n_mutations):
    ...
    for i in range(n_mutations):
        ...
        #     low, high
        # )  # This is the low/high of all variables and works best for benchmark functions with variables with same bounds
        random_number = random.uniform(
            low[indx[i]], high[indx[i]]
        )  # This is the low/high of each variable and works for functions with variables with different bounds (ex. antennas)
...
def find_new_candidates(
    ...
    with torch.no_grad():
        pred = predictor(
            torch.from_numpy(np.array(X_mutated_candidate)).float()
        )  # opou x candidate
        pred = pred.numpy()
        # min_value = pred.min()
...
for i_opt in range(n_opt):
    ...
    for iter in range(n_iterations):
        ...
        with open(
            ...
            Y_History_writer = csv.writer(
                Y_History_file, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
            )
            row = [str(iter + 1)]
            for i in range(n_dimensions):
